...
import { EditTrip, getEditTripById, updateEditTrip } from "@/common/db";
...
export default function SetTrips() {
  ...
  useLayoutEffect(() => {
    const fetchTrip = async () => {
      ...
      if (trip) {
        setTrips(trip);
        setSelectedPark(trip.parkId);
        setIsPublic(trip?.isPublic === undefined ? true : !!trip?.isPublic);
        setParkLocation(trip?.arrivalState!);
      }
      setIsLoading(false);
    };
    fetchTrip();
  ...
  const formik = useFormik({
    initialValues: {
      ...
      fare: trips?.fare || 0,
      date: trips?.departureDate || new Date().toISOString().split("T")[0], // Format the date for the input field
    },
    validationSchema,
    enableReinitialize: true, // This ensures the formik values update when trips change
    onSubmit: async (values: any) => {
      if (selectedPark && parkLocation) {
        ...
        pushWithUserTypePrefix(`/manage-trips/edit/preview/${tripId}`);
      } else {

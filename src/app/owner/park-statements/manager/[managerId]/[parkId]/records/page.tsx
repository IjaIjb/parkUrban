...
export default function Records({
  ...
  params: { managerId: string; parkId: string };
}) {
  console.log("🚀 ~ Records ~ params:", params);
  ...
  useEffect(() => {
    tripOBJs.managerRecords({ ...params, status: "scheduled" }).then((res) => {
    ...
    tripOBJs.managerRecords({ ...params, status: "completed" }).then((res) => {
      const transformData = res?.map((a: any) => ({
        startLocation: a.park.state,
        endState: a.endState,
        time: a.time,
